...
client.on("message", message => {
  ...
  if (
    msg.startsWith(`${prefix}SHOP`) ||
    msg.startsWith(`${prefix}STORE`) ||
    (msg.startsWith(`${prefix}BUY`) && !args[0])
  ) {
    let studentRole = {
      name: "Student Role",
      type: "Role",
      ...
      desc:
        "Gives you the `Student` role for access to the Academy channels on the Discord server."
    };
    ...
    getBal().then(() => {
      ...
      let catDescs = [
        "Spice up your name with a nifty color role!",
        "Get access to hidden sections of the Discord server  with special roles."
      ];
      let tingie;
      ...
      for (let i = 0; i < itemList.length; i++) {
        if (
          !nameList.includes(itemList[i]["name"]) ||
          !typeList.includes(itemList[i]["type"]) ||
          !priceList.includes(itemList[i]["price"]) ||
          !ridList.includes(itemList[i]["rid"]) ||
          !descList.includes(itemList[i]["desc"])
        ) {
      ...
      if (!args[0]) {
        const shopEmbed = new Discord.RichEmbed()
          .setAuthor(
            `You currently have ${maBal} Royal Gold!`,
            message.author.avatarURL
          )
          .addField(
            "Royal Store",
            "Looking to spend your hard-earned Royal Gold? You've come to the right place!\nI guarantee you will walk out with not a speck of gold in your pocket!"
          )
          .setColor(0xffbd1b);
